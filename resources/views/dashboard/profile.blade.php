        ...
        <div class="bg-white rounded-lg shadow-xl pb-8">
            ...
            <div class="flex flex-col items-center -mt-20">
                <img src="{{ auth()->user()->photo ? asset('storage/' . auth()->user()->photo) : asset('assets/img/default.png') }}"
                    class="w-40 border-4 border-white rounded-full" style="height: 160px; object-fit:cover;">
            ...
            <div class="flex-1 flex flex-col items-center lg:items-end justify-end px-8 mt-2">
                <div class="flex items-center space-x-4 mt-2">
                    ...
                    <form method="POST" action="{{ route('profile.destroy') }}" x-data="{ confirmDelete() { return confirm('¿Estás seguro de eliminar tu cuenta? Esta acción es irreversible.') } }"
                        @submit.prevent="if(confirmDelete()) { $el.submit() }">
                        @csrf
                        @method('DELETE')

                        <button type="submit"
                            class="flex items-center bg-blue-600 hover:bg-red-700 text-gray-100 px-4 py-2 rounded text-sm space-x-2 transition duration-100">
                            <svg class="h-4 w-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                                height="24" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
