    ...
    <div class="h-full bg-gray-200 p-8">
        <div class="m-auto bg-white p-4 max-w-5xl flex justify-center gap-8 rounded-lg shadow-xl py-8 mt-12">
            <div class="w-full px-6">
                <h4 class="text-xl font-bold capitalize text-gray-800">Editar Configuración</h4>
                ...
                <!-- Configuration Form -->
                <form action="{{ route('configuration.update', $config->id) }}" method="POST" id="configForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col mb-6">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="Ej: Configuración de usuarios" value="{{ old('name', $config->name) }}" required>
                    ...
                    <div class="mb-6">
                        ...
                        <div id="fieldsContainer">
                            @if (old('fields'))
                                ...
                                @endforeach
                            @else
                                @foreach ($config->structure as $index => $field)
                                    @include('dashboard.crud_configurations.field_template', [
                                        'index' => $index,
                                        'field' => $field,
                                    ])
                    ...
                    <div class="flex gap-2 mt-6">
                        ...
                        <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 rounded-lg px-6 py-2 text-white hover:shadow-xl transition duration-150">Actualizar
...
@section('custom-script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let fieldCount =
                {{ old('fields') ? count(old('fields')) : ($config->structure ? count($config->structure) : 0) }};
            ...
            addFieldButton.addEventListener('click', function() {
                ...
                fieldGroup.innerHTML = `
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="fields[${fieldCount}][course_start_date]"
                                   value="1" class="course-start-date rounded border-gray-300 text-blue-600 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                            <label class="ml-2 text-gray-700">Es fecha de inicio?</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" name="fields[${fieldCount}][course_end_date]"
                                   value="1" class="course-end-date rounded border-gray-300 text-blue-600 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                            <label class="ml-2 text-gray-700">Es fecha de culminación?</label>
                        </div>
                    </div>
                `;
                ...
                const startDateCheckbox = fieldGroup.querySelector('.course-start-date');
                const endDateCheckbox = fieldGroup.querySelector('.course-end-date');
                startDateCheckbox.addEventListener('change', () => handleDateCheckboxChange(
                    startDateCheckbox, 'course-start-date'));
                endDateCheckbox.addEventListener('change', () => handleDateCheckboxChange(endDateCheckbox,
                    'course-end-date'));
            ...
            document.querySelectorAll('.field-group').forEach(fieldGroup => {
                ...
                if (doctorSelect && hiddenInputs) {
                    doctorSelect.addEventListener('change', function() {
                        ...
                        if (value) {
                            ...
                            hiddenInput.type = 'hidden';
                            hiddenInput.name =
                                `fields[${fieldGroup.dataset.index}][${value}]`;
                            hiddenInput.value = 'true';
                            hiddenInputs.appendChild(hiddenInput);
                        }
                    });
                ...
                const startDateCheckbox = fieldGroup.querySelector('.course-start-date');
                const endDateCheckbox = fieldGroup.querySelector('.course-end-date');
                if (startDateCheckbox && endDateCheckbox) {
                    startDateCheckbox.addEventListener('change', () => handleDateCheckboxChange(
                        startDateCheckbox, 'course-start-date'));
                    endDateCheckbox.addEventListener('change', () => handleDateCheckboxChange(
                        endDateCheckbox,
                        'course-end-date'));
                }
            ...
            // Handle Date Checkbox Changes
            function handleDateCheckboxChange(checkbox, type) {
                const fieldGroup = checkbox.closest('.field-group');
                const otherCheckbox = fieldGroup.querySelector(
                    `.course-${type === 'course-start-date' ? 'end' : 'start'}-date`);

                // Prevent both start and end date being checked in the same field
                if (checkbox.checked && otherCheckbox.checked) {
                    otherCheckbox.checked = false;
                }

                // Ensure only one field can be start/end date
            ...
            document.querySelector('form').addEventListener('submit', function(event) {
                ...
                // Validate single start/end date
                const startDateCheckboxes = document.querySelectorAll('.course-start-date:checked');
                const endDateCheckboxes = document.querySelectorAll('.course-end-date:checked');

                if (startDateCheckboxes.length > 1) {
                    event.preventDefault();
                    alert('Solo puede haber un campo marcado como fecha de inicio.');
                    return;
                }

                if (endDateCheckboxes.length > 1) {
                    event.preventDefault();
                    alert('Solo puede haber un campo marcado como fecha de culminación.');
                    return;
                }

                // Validate no field has both start and end date
                document.querySelectorAll('.field-group').forEach(fieldGroup => {
                    const startDate = fieldGroup.querySelector('.course-start-date');
                    const endDate = fieldGroup.querySelector('.course-end-date');
                    if (startDate && endDate && startDate.checked && endDate.checked) {
                        event.preventDefault();
                        alert('Un campo no puede ser both fecha de inicio y fecha de culminación.');
                        return;
                    }
                });
