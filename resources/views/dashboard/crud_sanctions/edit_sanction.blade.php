    ...
    <div class="h-full bg-gray-200 p-8">
        ...
        <div class="bg-white rounded-lg shadow-xl pb-6 mb-6">
            <div class="flex flex-col md:flex-row items-center justify-between px-8 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div
                        class="p-3 rounded-full {{ $sanction->doctor->is_foreign ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Sanción #{{ $sanction->id }}</h1>
                        <div class="flex items-center space-x-4 mt-1">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                Médico: {{ $sanction->doctor->ci }}
                            </span>
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $sanction->doctor->is_foreign ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                                {{ $sanction->doctor->is_foreign ? 'Extranjero' : 'Nativo' }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-4 mt-4 md:mt-0">
                    <a href="{{ route('doctors.show', $sanction->doctor->id) }}"
                    ...
                    <form method="POST" action="{{ route('sanctions.destroy', $sanction->id) }}" x-data="{ confirmDelete() { return confirm('¿Estás seguro de eliminar esta sanción? Esta acción es irreversible.') } }"
        ...
        <div class="my-4 flex flex-col lg:flex-row gap-6">
            ...
            <div class="w-full lg:w-2/3 bg-white rounded-lg shadow-xl p-6">
                <h4 class="text-xl text-gray-900 font-bold mb-6">Información del Médico Sancionado</h4>
            ...
            <!-- Columna derecha: Edición de la sanción -->
            <div class="w-full lg:w-1/3 bg-white rounded-lg shadow-xl p-6 h-fit">
                <h4 class="text-xl text-gray-900 font-bold mb-6">Editar Sanción</h4>

                <!-- Formulario para editar la razón -->
                <form action="{{ route('sanctions.update', $sanction->id) }}" method="POST">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 mb-6">
                        <div class="flex items-center mb-3">
                            ...
                            <span class="font-semibold text-gray-700">Información de la Sanción</span>
                        </div>

                        <div class="space-y-4">
                            <!-- Fecha de inicio -->
                            <div>
                                <label class="font-medium text-gray-700 text-sm">Fecha de Inicio</label>
                            ...
                            <!-- Fecha de fin -->
                            <div>
                                <label class="font-medium text-gray-700 text-sm">Fecha de Fin</label>
                                <p class="text-gray-900">
                            ...
                            <!-- Razón (editable) -->
                            <div>
                                <label for="reason" class="font-medium text-gray-700 text-sm">Razón de la Sanción
                                    *</label>
                                <textarea name="reason" id="reason" rows="4" required
                                    ...
                                    placeholder="Describa la razón de la sanción">{{ old('reason', $sanction->reason) }}</textarea>
                                @error('reason')
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 mt-6">
                    ...
                    <div class="space-y-2">
                        <p class="text-sm text-gray-600">Creado: {{ $sanction->created_at->format('d/m/Y H:i') }}</p>
                        <p class="text-sm text-gray-600">Actualizado: {{ $sanction->updated_at->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
